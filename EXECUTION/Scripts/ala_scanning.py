...
# We use the module Path, so that is not needed to change the path if we execute from another computer
home = str(Path.home())
base_path = os.path.join(home, "Downloads", "EXECUTION/")
...
# We wrote the outputs in a file named output.txt
f = open("output.txt", "a")
f.write("\nTotal energy of all residues, alanine scanning: ")
f.write(str(Telec + Tvdw + Tsolvation - TsubunitA - TsubunitE))
f.write("\nTotal energy of interface residues, alanine scanning: ")
f.write(str(Ielec + Ivdw + Isolvation - IsubunitA - IsubunitE))
