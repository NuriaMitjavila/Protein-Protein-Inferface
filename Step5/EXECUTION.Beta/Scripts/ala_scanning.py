...
home = str(Path.home())
base_path = os.path.join(home, "Downloads", "EXECUTION.Beta/")

residue_library = ResiduesDataLib(base_path + 'Files/aaLib.lib')
ff_params = VdwParamset(base_path + 'Files/vdwprm.txt')
pdb_path = base_path + "8df5_fixed.pdb"
parser = PDBParser(PERMISSIVE=1)
st = parser.get_structure('st', pdb_path)
add_atom_parameters(st, residue_library, ff_params)
ala_atoms = {'N', 'H', 'CA', 'HA', 'C', 'O', 'CB', 'HB', 'HB1', 'HB2', 'HB3', 'HA1', 'HA2', 'HA3'}

NACCESS_BINARY = base_path + 'Files/soft/NACCESS/naccess'
...
for residues in chainF.get_residues():
    Telec += calc_int_energies(st[0], residues)[0]
    Tvdw += calc_int_energies(st[0], residues)[1]
    Tsolvation += calc_solvation(residues)
    TsubunitF += calc_solvation(st[0][chainF.id][residues.id[1]])
    diccionari[residues.id[1]] = calc_int_energies(st[0], residues)[0]+calc_int_energies(st[0], residues)[1]+calc_solvation(residues)-calc_solvation(st[0][chainF.id][residues.id[1]])

for residues in chainS.get_residues():
    Telec += calc_int_energies(st[0], residues)[0]
    Tvdw += calc_int_energies(st[0], residues)[1]
    Tsolvation += calc_solvation(residues)
    TsubunitS += calc_solvation(st[0][chainS.id][residues.id[1]])
    diccionari[residues.id[1]] = calc_int_energies(st[0], residues)[0]+calc_int_energies(st[0], residues)[1]+calc_solvation(residues)-calc_solvation(st[0][chainS.id][residues.id[1]])

print("Total energy of all residues: ", Telec + Tvdw + Tsolvation - TsubunitF - TsubunitS)
...
for residues in get_interface(st, 7)["F"]:
    Ielec += calc_int_energies(st[0], residues)[0]
    Ivdw += calc_int_energies(st[0], residues)[1]
    Isolvation += calc_solvation(residues)
    IsubunitF += calc_solvation(st[0][chainF.id][residues.id[1]])

for residues in get_interface(st, 7)["S"]:
    Ielec += calc_int_energies(st[0], residues)[0]
    Ivdw += calc_int_energies(st[0], residues)[1]
    Isolvation += calc_solvation(residues)
    IsubunitS += calc_solvation(st[0][chainS.id][residues.id[1]])

print("Total energy of interface residues: ", Ielec + Ivdw + Isolvation - IsubunitF - IsubunitS)
...
for x in pos_important:
    for residues in st.get_residues():
        if residues.id[1] == x:
            ...
            saveP.append(residues.get_resname())
...
for x in neg_important:
    for residues in st.get_residues():
        if residues.id[1] == x:
            print(residues.get_resname())
            saveN.append(residues.get_resname())
...
fig1.savefig('Figure_1.jpg', dpi=100)



f = open("output.txt", "a")
f.write("\nTotal energy of all residues, alanine scanning: ")
f.write(str(Telec + Tvdw + Tsolvation - TsubunitF - TsubunitS))
f.write("\nTotal energy of interface residues, alanine scanning: ")
f.write(str(Ielec + Ivdw + Isolvation - IsubunitF - IsubunitS))
f.write('\nPositive energy residues: ')
for item in saveP:
    f.write(item)
    f.write(", ")
f.write('\nNegative energy residues: ')
for item in saveN:
    f.write(item)
    f.write(", ")
f.close()
